...
export default function TaskFlowBoard() {
  ...
  allParticipants.forEach((p) => {
    // Map 'submitted' status to 'completed' (Pending Review) stage
    const mappedStatus = p.status === 'submitted' ? 'completed' : p.status;
    const s = mappedStatus as FlowStage;
  ...
  const getParticipantsForTaskAndStage = (
    ...
    return allParticipants.filter(
      (p) => {
        const mappedStatus = p.status === 'submitted' ? 'completed' : p.status;
        return p.task_id === taskId && mappedStatus === stage;
      }
    );
  };
  ...
  const getTaskStages = (taskId: string): FlowStage[] => {
    ...
    taskParts.forEach((p) => {
      // Map 'submitted' to 'completed' for stage tracking
      const mappedStatus = p.status === 'submitted' ? 'completed' : p.status;
      if ((STAGES.map((s) => s.key) as string[]).includes(mappedStatus)) {
        stages.add(mappedStatus as FlowStage);
      }
    });
